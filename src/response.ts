...
export class CdpSession implements CdpClient {
  ...
  send<T = unknown>(
    ...
    return new Promise((resolve, reject) => {
      ...
      // The WHATWG `send` reports failure by throwing synchronously rather than
      // through a completion callback, so the pending entry has to be unwound
      // here or the command would sit until its timeout with no socket to
      // answer it.
      try {
...
export class ResponseMonitor {
  ...
  async authenticationStatus(): Promise<ChatGptAuthenticationStatus> {
    ...
    if (hasSessionCookie) {
      return {
        authenticated: true,
        detail: "ChatGPT account session detected in the isolated managed profile."
      };
    }
    ...
    return controls.hasAccountControl && !controls.hasLoginControl
      ? {
          authenticated: true,
          detail: "ChatGPT account controls detected in the isolated managed profile."
        }
      : {
          authenticated: false,
