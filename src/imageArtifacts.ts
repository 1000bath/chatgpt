...
import type { ImageArtifact, SupportedImageMimeType } from "./port.js";
...
export async function persistBrowserImages(
  ...
  if (!path.isAbsolute(artifactsDir)) {
    throw new ChatGptBrowserError(
      ...
      "Pass an absolute path as `artifactsDir` on the request."
    );
  }
  ...
  if (relative.startsWith("..") || path.isAbsolute(relative)) {
    throw new ChatGptBrowserError(
      ...
      "Inspect the `artifactsDir` passed on the request."
    );
  }
