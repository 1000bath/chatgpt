...
export class ChatGptBrowserBackend implements ExecutionBackend {
  ...
  private async runOnceOnDedicatedWindow(
    ...
    try {
      ...
      if (err instanceof ChatGptBrowserError) {
        if (err.code === "CHATGPT_BROWSER_CHALLENGE_REQUIRED" && screenshotPath) {
          throw new ChatGptBrowserError(
            err.code,
...
export function classifyBrowserError(error: unknown): "transient" | "permanent" {
  if (error instanceof ChatGptBrowserError) {
    if (
      error.code === "CHATGPT_BROWSER_RATE_LIMITED"
      || error.code === "CHATGPT_BROWSER_CHALLENGE_REQUIRED"
      || error.code === "CHATGPT_BROWSER_UNSUPPORTED_PLATFORM"
      || error.code === "CHATGPT_BROWSER_MODE_DISABLED"
    ) return "permanent";
    if (error.code === "CHATGPT_BROWSER_EXECUTION_FAILED") {
