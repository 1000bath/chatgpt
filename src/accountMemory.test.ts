...
describe("ChatGPT account memory", () => {
  ...
  test("requires an exact save confirmation", () => {
    expect(isAccountMemorySaveConfirmed(" CHATGPT_MEMORY_SAVED\n")).toBe(true);
    expect(isAccountMemorySaveConfirmed("I saved it. CHATGPT_MEMORY_SAVED")).toBe(false);
    expect(isAccountMemorySaveConfirmed("CHATGPT_MEMORY_NOT_SAVED: disabled")).toBe(false);
  });
