 ...
 *
 * The launch flags above prevent the freeze for instances this package starts, but a
...
export class ChromeLauncher {
  ...
  private async launchLocked(config: ChatGptBrowserConfig): Promise<ChromeProcessInfo> {
    ...
    const args = [
      ...
      // The managed Chrome window is an automation surface nobody interacts with,
    ...
    try {
      ...
      if (new URL(wsUrl).pathname !== launchedEndpoint.browserPath) {
        throw new Error("Chrome DevTools endpoint did not match the isolated managed profile.");
      }
      return {
...
export async function openChatGptInManagedChrome(
 ...
 * exact managed profile. Returns false when no verified instance is running.
 */
export async function closeActiveManagedChrome(profileDir: string): Promise<boolean> {
  ...
  throw new Error("The managed Chrome instance did not close within 10 seconds.");
}
...
export async function openChatGptForManualLogin(
  ...
  await closeActiveManagedChrome(config.profileDir);
